...
code = r'''import subprocess, json, os, time, concurrent.futures
from datetime import date

TOOL = r"C:\Users\Administrator\.qclaw\workspace\china-stock-data\tool_runner.py"

def call(tool, **params):
    args = ["python", TOOL, tool]
    for k in params:
        args.append(k + "=" + str(params[k]))
    try:
    ...
    with open(fp, "rb") as f:
        while True:
            ...
            if len(d) < 32:
                break
            try:
                di_be, o, h, l, c = struct.unpack(">Iffff", d[:20])
            except:
                break
            if di_be < 20100101 or di_be > 20991231 or c <= 0:
                continue
            y, r2 = divmod(di_be, 10000)
            m, day = divmod(r2, 100)
            if day < 1 or day > 31:
                day = 1
            try:
...
def run():
    ...
    zt = call("tool_fetch_limit_up_stocks")
    info = {}
    zt_list = []
    for s in zt[:50]:
        code = s.get("code", "")
        ...
        if not code or code in info:
            continue
        lp = round(float(s.get("latest_price", 0) or 0), 2)
        cp = round(float(s.get("change_pct", 0) or 0), 2)
        info[code] = {"n": name, "b": board, "p": lp, "tp": cp,
                       "ct": s.get("continuous_limit_up_count", 0),
                       "lt": s.get("limit_up_time", "")}
        zt_list.append({"name": name, "code": code,
                       "pct": round(float(s.get("change_pct", 0) or 0), 0),
                       "board": board, "reason": board or "涨停", "price": lp,
                       "today_pct": cp,
    ...
    def job(code):
        ...
        fp = r"D:\通达信三点一线专用版\vipdoc\\" + mk + "\\lday\\" + mk + code + ".day"
        bars = parse_day(fp)
    ...
    print("Done in", round(time.time() - t0, 1), "s |", now)
    for s in month_top[:10]:
        print(str(s["rank"]) + ". " + s["name"] + "(" + s["code"] + ") 月涨" + str(s["pct"]) + "% 今涨" + str(s.get("today_pct", 0)) + "% [" + s.get("board", "") + "]")
    print("ZT:", len(zt_list), "只")
    return {"updated_at": now, "month_top": month_top[:15], "zt_today": zt_list}

if __name__ == "__main__":
    data = run()
    out = os.path.join(os.path.dirname(os.path.abspath(__file__)), "snapshot_v4.json")
    with open(out, "w", encoding="utf-8") as f:
        json.dump(data, f, ensure_ascii=False, indent=2)
'''

# Write
target = r"C:\Users\Administrator\.qclaw\workspace\china-stock-data\run.py"
